...
WITH meds AS (
...
), med_pairs /* Self-join to find medication pairs per patient */ AS (
  SELECT
    a.patient_id,
    ...
    CASE
      WHEN (
        a.medication_name = 'Enoxaparin' AND b.medication_name = 'Aspirin'
      )
      THEN 'HIGH'
      WHEN (
        a.medication_name LIKE '%statin' AND b.medication_name LIKE '%fibrate'
      )
      THEN 'HIGH'
      WHEN (
        a.route = 'IV' AND b.route = 'IV'
      )
      THEN 'MODERATE'
      ELSE 'LOW'
    END AS interaction_risk /* Known interaction rules */
  FROM meds AS a
  JOIN meds AS b
  ...
  SELECT
    ...
    ARRAY_JOIN(SORT_ARRAY(COLLECT_SET(medication_name)), ', ') AS all_medications,
    SORT_ARRAY(ARRAY_AGG(DISTINCT route)) AS routes_array,
...
SELECT
  ...
  IF(ps.total_medications >= 5, TRUE, FALSE) AS is_polypharmacy
FROM patient_summary AS ps
LEFT JOIN med_pairs AS mp
  ON ps.patient_id = mp.patient_id
GROUP BY
  ps.patient_id,
  ps.total_medications,
  ps.injectable_count,

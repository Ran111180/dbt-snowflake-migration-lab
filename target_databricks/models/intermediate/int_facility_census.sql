...
WITH daily_census AS (
  SELECT
    ...
    APPROX_PERCENTILE(length_of_stay, 0.90) AS p90_los,
...
SELECT
  ...
  IF(total_patients = 0 OR total_patients IS NULL, 0, active_count / total_patients) AS active_rate, /* Utilization metrics */
  IF(
    total_patients = 0 OR total_patients IS NULL,
    0,
    complex_patient_count / total_patients
  ) AS complexity_ratio,
  LAG(total_patients) OVER (PARTITION BY facility_id ORDER BY admit_date ASC NULLS LAST) AS prev_day_total, /* Trend vs previous admit date */
  total_patients - COALESCE(
    LAG(total_patients) OVER (PARTITION BY facility_id ORDER BY admit_date ASC NULLS LAST),
    total_patients
  ) AS daily_change
FROM daily_census
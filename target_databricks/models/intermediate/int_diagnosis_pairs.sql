...
WITH dx AS (
...
), pairs /* Generate all diagnosis pairs per patient */ AS (
  SELECT
    a.patient_id,
  ...
  FROM dx AS a
  JOIN dx AS b
    ON a.patient_id = b.patient_id AND a.icd10_code < b.icd10_code
)
SELECT
  dx_1,
  dx_2,
  ...
  (COUNT(DISTINCT patient_id)) / SUM(COUNT(DISTINCT patient_id)) OVER () AS prevalence_pct,
  SORT_ARRAY(ARRAY_AGG(DISTINCT patient_id)) AS patient_ids
FROM pairs
GROUP BY
  dx_1,
  dx_2,

...
WITH date_series AS (
  SELECT
    DATEADD(DAY, id, DATEADD(DAY, -90, CURRENT_DATE())) AS forecast_date
  FROM (SELECT EXPLODE(SEQUENCE(0, 119)) AS id)
), daily_census AS (
  SELECT
    facility_id,
  ...
  SELECT
    ...
    AVG(COALESCE(dc.patient_count, 0)) OVER (
      ...
      ORDER BY ds.forecast_date ASC NULLS LAST
  ...
  LEFT JOIN daily_census AS dc
    ON dc.facility_id = f.facility_id AND dc.census_date = ds.forecast_date
)
SELECT
  facility_id,
  forecast_date,
  ...
  IF(forecast_date > CURRENT_DATE(), TRUE, FALSE) AS is_forecast,

...
SELECT
  ...
  readmissions,
  total_encounters,
  IF(total_encounters = 0 OR total_encounters IS NULL, 0, readmissions / total_encounters) AS readmission_rate,
  CURRENT_TIMESTAMP() AS _last_updated
FROM source
QUALIFY
  ROW_NUMBER() OVER (PARTITION BY facility_id, week_start ORDER BY total_encounters DESC NULLS FIRST) = 1
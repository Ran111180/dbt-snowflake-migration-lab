...
WITH source AS (
    SELECT
        ...
        COUNT_IF(length_of_stay > {{ var('los_long_stay') }}) AS long_stay_count
...
SELECT
    {{ surrogate_key(['facility_id', 'census_date']) }} AS census_key,
    facility_id,
    census_date,
    patient_count,
    active_patients,
    ...
    elderly_85_plus,
    long_stay_count,
    {{ safe_divide('active_patients', 'patient_count') }} AS occupancy_rate,
